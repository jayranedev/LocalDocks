...
function ProcessRowItem({
  ...
  return (
    <button
      ...
      className={`group flex h-[42px] w-full items-center gap-[13px] border-b border-border px-3.5 text-left transition-colors last:border-b-0 hover:bg-surface-selected ${
        selected ? 'bg-surface-selected' : ''
      }`}
    >
      <StatusDot tone={process.status === 'running' ? 'success' : 'muted'} />
      <span
        className={`flex w-[192px] items-center gap-1.5 font-mono text-[11.5px] ${
          process.isService ? 'text-primary' : 'text-muted'
        }`}
      >
        <span className="truncate">{process.name}</span>
        {process.isService && <Chip tone="quiet">svc</Chip>}
      </span>
      <span className="w-[60px] font-mono text-[11.5px] text-muted tabular-nums">{process.pid}</span>
      <span className="w-[60px] font-mono text-[11.5px] text-muted tabular-nums">
        {process.parentPid}
      </span>
      <span className="flex-1" />
      <span className="w-[56px] text-right font-mono text-[11.5px] text-secondary tabular-nums">
        {formatCpu(process.cpuPercent)}
      </span>
      <span className="w-[70px] text-right font-mono text-[11.5px] text-primary tabular-nums">
